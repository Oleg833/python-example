...
def vector2(x1, y1, x2, y2):
    c.create_line(xn/2+x1*step, yn/2-y1*step, xn/2+x2*step, yn/2-y2*step, fill='blue',
                  width=5, arrow=LAST,
                  activefill='blue',
                  arrowshape="10 20 10")


def vector3(x1, y1, x2, y2):
    c.create_line(xn/2+x1*step, yn/2-y1*step, xn/2+x2*step, yn/2-y2*step, fill='red',
                  width=5, arrow=LAST,
                  activefill='blue',
                  arrowshape="10 20 10")


def vector4(x1, y1, x2, y2):
    c.create_line(xn/2+x1*step, yn/2-y1*step, xn/2+x2*step, yn/2-y2*step, fill='yellow',
                  width=5, arrow=LAST,
                  activefill='blue',
                  arrowshape="10 20 10")
...
def vector_mult(x1, y1, x2, y2, k):
    ...
    vector_color(k*x1, k*y1, k*x2, k*y2, 'brown')


def cos_fi(a_x1, a_y1, a_x2, a_y2, b_x1, b_y1, b_x2, b_y2):
    vector_color(a_x1, a_y1, a_x2, a_y2, 'brown')
    vector_color(b_x1, b_y1, b_x2, b_y2, 'yellow')
    c.create_rectangle(xn-350, yn-20, xn, yn, outline="#fb0", fill="#ddd")
    a_x = a_x2-a_x1
    a_y = a_y2-a_y1
    b_x = b_x2-b_x1
